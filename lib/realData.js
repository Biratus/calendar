...
import { areIntervalsOverlapping, formatISO } from "date-fns";
...
export function fetchFiliere(filiereId) {
  return modules.filter(m => m.filiere == filiereId);
}

export function getModulesOfFormateur(formateurId,interval) {
  return modules.filter(m => m.formateur.mail == formateurId && areIntervalsOverlapping({start:m.start,end:m.end},interval));
}
