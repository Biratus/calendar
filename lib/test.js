...
import { mapISO, mergeModule } from "./calendar";
...
export function mergeModuleTest() {
  ...
  let newEvent = mergeModule(m1, m2);
  ...
  console.assert(newEvent.end == m2.end,`End: excpected ${m2.end} but got ${newEvent.end}`)
...
export function doubleMergeModuleTest() {
  ...
  let newEvent = mergeModule(m1, m2);
  newEvent = mergeModule(newEvent, m3);
