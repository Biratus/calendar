...
export default function handler(req,res) {
    ...
    if(isPut(req)) {
        ...
        for(let f of formateurs) {
            if(!formateurReal.hasOwnProperty(f.mail)) return requestError(res,`Pas de formateur avec mail [${f.mail}]`)
        }

        // Check date;
        if(!isWithinInterval(split,{start:parseISO(mod.start),end:parseISO(mod.end)})) {
            return requestError(res,"La date de coupure est en dehors des dates du module")
        }
        ...
        m1.formateur = formateurReal[formateurs[0].mail];
        m2.formateur = formateurReal[formateurs[1].mail];
        m2.id = uuidv4();
